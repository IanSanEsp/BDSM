...
// Asi como poner formato para nombres de profesor
...
export const importFullDb = async (req, res) => {
  try {
    ...
    for (const h of horarios) {
      ...
      try {
        // Excel ya tiene id_materia en hoja horario_grupo usar directamente
    ...
    // Salon markers, deprecado creo nose
...
async function ensureSalon(nombre) {
  ...
  // Crear salon con valores defecto
  ...
  try {
    ...
    if (req.file) {
      ...
      for (const row of jsonData) {
        ...
        if (!salon || !materia) continue; // filas incompletas
      ...
      if (jsonData.length > 0 && jsonData[0].Id_salon) {
        ...
        for (const row of jsonData) {
          ...
          // id_materia
          const [materiaRows] = await db.query('SELECT id_materia FROM grupo_materia WHERE id_grupo = ? LIMIT 1', [id_grupo]);
          if (!materiaRows || materiaRows.length === 0) continue;
